...
def stay_in_ball_area_but_not_too_close_heuristic(game, start):
...
def stay_in_ball_area_but_not_too_close_no_admissible_x_axis_heuristic(game, start):
    if not game.balls and not game.hexagons:
        return 0
...
def stay_in_ball_area_but_not_too_close_no_admissible_x_axis_danger_both_axis_heuristic(game, start):
    if not game.balls and not game.hexagons:
        return 0
...
def player_bonus_and_ball_heuristic(game, start):
    if not game.balls and not game.hexagons:
        return 0
    agent_dist = find_the_distance_from_the_closest_ball_at_x_axis(game)[1]
    if agent_dist < DANGER_X_DIST_FROM_BUBBLE:
